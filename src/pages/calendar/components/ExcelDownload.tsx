...
function ExcelDownload(props: Props) {

    const { t } = useTranslation();
    const [downloadLink, setDownloadLink] = useState<string>();

    const addTaskRows = (subtitle: string, tasks: TaskT[], sheet: Worksheet) => {
    ...
    useEffect(() => {
        const generateExcel = async (tasks: TaskT[]): Promise<string> => {
            ...
            row = sheet.addRow({ when: t('calendarPage.excel.firstCampDay'), what: props.startOfCamp })
            row.eachCell((cell, _) => {
                cell.border = thinBorders;
            ...
            const tasksBeforeCamp = tasks.filter(t => isBefore(t.deadline, props.startOfCamp))
        ...
        const prepareDownloadLink = async () => {
            ...
            const downloadLink = await generateExcel(props.tasks)
    ...
    return (
        <a className="download-btn" id="excel-download"
           download={t('calendarPage.excel.filename', { calendarTitlePrefix: spacedCalendarPrefix })}
           href={downloadLink}>
            <FontAwesomeIcon icon={faFileExcel}/>
            {t('calendarPage.excel.download')}
        </a>
    );
}
